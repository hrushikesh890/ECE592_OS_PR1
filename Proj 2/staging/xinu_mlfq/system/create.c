                     ...
                     uint32 sleep_duration)
{
	...
	//uint32 tracker;
	mask = disable();
	kprintf("\n%d . %u %u %u\n\n", currpid, number_bursts, burst_duration, sleep_duration);
restore(mask);	
	for (i = 0; i < number_bursts; i++)
	{
		mask = disable();
		//kprintf("(%d). %u %u %u runtime %u val %u\n", currpid, number_bursts, i, burst_duration, proctab[currpid].runtime, ((i+1)*burst_duration));
		restore(mask);		
