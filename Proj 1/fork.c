...
/*pid32 fork()
{
	...
	parentPID = currpid;
	kprintf("currPid = %d %s %d 0x%08X", currpid, proctab[currpid].prname, proctab[currpid].prparent, proctab[currpid].prstkbase);
...
pid32 create_child(struct procent *parentptr)
{
	uint32		savsp, *pushsp;
	...
	prptr->prstklen = parentptr->prstklen;
